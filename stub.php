...
if (version_compare(phpversion(), '5.3.1', '<')) {
    if (substr(phpversion(), 0, 5) != '5.3.1') {
        ...
        echo "PEAR2_Acme_Pyrus_Xmas requires PHP 5.3.1 or newer.
";
        exit -1;
    }
}
...
try {
    ...
    echo "Cannot process PEAR2_Acme_Pyrus_Xmas phar:
";
    echo $e->getMessage(), "
";
    exit -1;
}
function PEAR2_Acme_Pyrus_Xmas_autoload($class)
{
    $class = str_replace(array('_', '\\'), '/', $class);
    if (file_exists('phar://' . __FILE__ . '/PEAR2_Acme_Pyrus_Xmas-0.4.0/php/' . $class . '.php')) {
        include 'phar://' . __FILE__ . '/PEAR2_Acme_Pyrus_Xmas-0.4.0/php/' . $class . '.php';
    }
}
spl_autoload_register("PEAR2_Acme_Pyrus_Xmas_autoload");
$phar = new Phar(__FILE__);
$sig = $phar->getSignature();
define('PEAR2_Acme_Pyrus_Xmas_SIG', $sig['hash']);
define('PEAR2_Acme_Pyrus_Xmas_SIGTYPE', $sig['hash_type']);
